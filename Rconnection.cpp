...
int Rconnection::login(const char *user, const char *pwd) {
  ...
  const char *crypt_res = NULL;
#ifdef unix
  if (auth&A_crypt) {
    crypt_res = crypt(pwd, salt);
    if (!crypt_res) return CERR_auth_unsupported;
  }
#else
  if (!(auth&A_plain)) {
  ...
  size_t buf_size = strlen(user) + 2; // user + \n + \0
  if (crypt_res) {
    buf_size += strlen(crypt_res);
  } else {
    buf_size += strlen(pwd);
  }

  authbuf=(char*) malloc(buf_size);
